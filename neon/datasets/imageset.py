...
class MacrobatchDecodeThread(Thread):
    ...
    def run(self):
        ...
        uimg_macro = np.zeros((self.ds.macro_size, self.ds.npixels),
                              dtype=np.uint8)
        self.ds.img_macro[b_idx] = np.zeros((self.ds.macro_size, self.ds.npixels),
                              dtype=betype)
        ...
        for mini_idx in range(self.ds.minis_per_macro[b_idx]):
            ...
            for lbl in self.ds.label_list:
                ...
                self.ds.lbl_one_hot[b_idx][lbl][mini_idx] = \
                    np.eye(self.ds.nclass[lbl])[hl].astype(betype, order='C')
    ...
    def init_mini_batch_producer(self, batch_size, setname, predict=False):
        ...
        if self.mean_norm:
            self.mean_val = self.mean_crop.reshape(
                            (1, self.npixels)).astype(np.float32)
        else:
            self.mean_val = 127
        ...
        self.img_macro = [None for i in range(self.macro_num_decode_buf)]
