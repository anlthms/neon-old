# import shortcuts
try:
    from neon.models.autoencoder import Autoencoder  # noqa
    from neon.models.balance import Balance  # noqa
    from neon.models.dbn import DBN  # noqa
    from neon.models.gb import GB  # noqa
    from neon.models.mlp import MLP, MLPL  # noqa
    from neon.models.rbm import RBM  # noqa
    from neon.models.rnn import RNN  # noqa
    from neon.models.gb_dist import GBDist  # noqa
    from neon.models.convnet_dist import ConvnetDist  # noqa
except ImportError:
    pass
