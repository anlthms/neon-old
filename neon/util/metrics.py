...
def misclass_sum(backend, outputs, targets, predlabels, labels,
                 misclass, retval):
...
def auc(backend, outputs, targets):
    return metrics.roc_auc_score(targets.asnumpyarray().ravel(),
                                 outputs.asnumpyarray().ravel())


def logloss(backend, outputs, targets, sums, temp, retval, eps=1e-15):
