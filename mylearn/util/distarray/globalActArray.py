...
class GlobalActArray(object):
    ...
    def initializeHalos(self):           
        ...
        #haloInsertIndex = self.localActArray.localArraySize
        
        for k in self.localActArray.haloIds:
            if k == -1: 
                return
            
            neighborActArrayIndex = [gc.posOffsets[k][0] + i, gc.posOffsets[k][1]+ j] 
            #figure out send halo indices and recv halo size
            self.localActArray.setHaloIndices(k, neighborActArrayIndex)
            #haloInsertIndex += curHaloSize #insert location is in sorted borderId order
        
            if self.printDebug:
                 print 'Setting up halos to send for block: ', i, ',', j, ' ', gc.haloDirNames[k], 'haloIndices: ', self.localActArray.sendHalos[k].haloIndices 

        self.localActArray.computeHaloInsertIndices()
        #self.localActArray.allocateLocalMemory()    
        #self.localActArray.sendRecvHalos()
        #self.localActArray.makeConsistentLocalChunk()
                
    
    def __init__(self, batchSize=1, actSize=8, actChannels=1, filterSize=2, backend=None):
        ...
        pprint("Creating a %d x %d processor grid..." % (commPerDim, commPerDim) )
        #if layerId ==0:
        ...
        haloSizeRow = (filterSize - 1) // 2 + ( (filterSize - 1) %2 > i ) #dimensions along up/down axis
        w = actSize // commPerDim + (actSize % commPerDim > j)
        haloSizeCol = (filterSize - 1) // 2 + ( (filterSize - 1) %2 > j ) #dimensions along left/right axis
        borderId = self.computeBorder(i,j,commPerDim)
        topLeftRow = h*i
        topLeftCol = w*j
        self.localActArray = laa.LocalActArray(batchSize, i, j, h, w, actChannels, topLeftRow, topLeftCol, 
        ...
        for k in self.localActArray.haloIds:
            ...
            sendDims = [self.localActArray.width, self.localActArray.height, self.localActArray.actChannels, self.localActArray.haloSizeRow, self.localActArray.haloSizeCol ]
            comm.Isend(np.asarray(sendDims, dtype='int32'), neighborCommIndex)
            #recvTag = neighborActArrayIndex[0]*100 + neighborActArrayIndex[1]*10 + k
